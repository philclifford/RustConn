...
- Improved: the checks that are supposed to refuse a bad release stopped being
  able to pass without looking. The lint gate reported no problems from a
  cached result, and the release script ended a run that had skipped three of
  its checks the same way it ended one that ran all of them. Both now say what
