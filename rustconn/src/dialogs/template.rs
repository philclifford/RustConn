...
pub struct TemplateDialog {
    ...
    ssh_identities_only: adw::SwitchRow,
    ssh_control_master: adw::SwitchRow,
    ssh_agent_forwarding: adw::SwitchRow,
...
impl TemplateDialog {
    ...
    fn create_ssh_options() -> (
        ...
        let agent_forwarding = adw::SwitchRow::builder()
            .title(i18n("Agent Forwarding"))
            .subtitle(i18n("Forward SSH agent to remote host (-A)"))
            .build();
        session_group.add(&agent_forwarding);
    ...
    fn create_vnc_options() -> (
        ...
        // Scaling
        let scaling_check = adw::SwitchRow::builder()
            .title(i18n("Scale Display"))
            .subtitle(i18n("Scale display to fit window"))
            .active(true)
            .build();
        display_group.add(&scaling_check);
        ...
        // View only
        let view_only_check = adw::SwitchRow::builder()
            .title(i18n("View Only"))
            .subtitle(i18n("Disable keyboard and mouse input"))
            .build();
        features_group.add(&view_only_check);
    ...
    fn create_spice_options() -> (
        ...
        tls_check.connect_active_notify(move |check| {
            ...
            skip_verify_check_clone.set_visible(is_tls);
        });

        // Initial state: TLS disabled - hide related fields
        ca_cert_row.set_visible(false);
        skip_verify_check.set_visible(false);
    ...
    fn build_ssh_config(
        ...
        key_entry: &Entry,
        proxy_entry: &Entry,
        identities_only: &adw::SwitchRow,
        control_master: &adw::SwitchRow,
        agent_forwarding: &adw::SwitchRow,
        startup_entry: &Entry,
        options_entry: &Entry,
    ) -> ProtocolConfig {
    ...
    fn build_vnc_config(
        ...
        view_only_check: &adw::SwitchRow,
        scaling_check: &adw::SwitchRow,
        clipboard_check: &adw::SwitchRow,
        custom_args_entry: &Entry,
    ) -> ProtocolConfig {
        let client_mode = if client_mode_dropdown.selected() == 1 {
    ...
    fn build_spice_config(
        tls_check: &adw::SwitchRow,
        ca_cert_entry: &Entry,
        skip_verify_check: &adw::SwitchRow,
        usb_check: &adw::SwitchRow,
        clipboard_check: &adw::SwitchRow,
