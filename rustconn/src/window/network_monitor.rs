...
pub fn setup_network_monitor(
    ...
    monitor.connect_network_changed(move |mon, available| {
        ...
        if change == NetworkChange::Baseline {
            ...
            // Still health-check existing sockets: outside Flatpak the monitor
            // may not emit an initial signal at all, so the first real event
            // (VPN connect on a live network) lands here. A socket check is
            // cheap and safe even when nothing is stale.
