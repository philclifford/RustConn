...
- **SSH through a jump host could hang at the target password prompt when the proxy step was slow (issue [#301](https://github.com/totoshko88/RustConn/issues/301))** — the bastion password is delivered out of band through `SSH_ASKPASS`, but the target host's password is typed by a watcher that recognises the `password:` prompt in the terminal. That watcher gave up after a fixed 10-second window measured from the moment SSH was spawned, so the whole `ProxyCommand` handshake to the bastion ate into the same budget. When the proxy step ran long, the watcher had already stopped by the time the target prompt appeared, and the session sat at `password:` doing nothing. The window is now measured from the last terminal activity instead of from spawn: as long as the connection is still making progress the watcher keeps waiting, and it only gives up after the terminal has been genuinely idle for the timeout (default 10 s, still overridable per connection or group), with a 120-second absolute ceiling. Nothing is typed twice — the one-shot behaviour that stops a wrong password from locking an account is unchanged.
