...
- **Portable encrypted file backend — cloud-syncable credential storage (issue [#293](https://github.com/totoshko88/RustConn/issues/293))** — a secret backend keyed by a passphrase you choose instead of by the machine, so one file holds your passwords and opens on every machine you own, Linux and macOS alike. The reporter's problem was that no existing option worked between the two, which had kept them on Asbru-CM. Configured in Settings ▸ Secrets as *Portable encrypted file (cloud-syncable)*: pick a path inside whatever folder your cloud client syncs, choose a passphrase, and optionally have it remembered locally (machine-encrypted file or system keyring). **Existing passwords ▸ Copy Credentials** re-encrypts the credentials already on this machine into the portable file, keeping the originals as this machine's fallback. A connection that needs a credential while the passphrase is not in memory raises an unlock dialog rather than failing. `rustconn-cli secret get`/`set`/`delete` work with it too, prompting for the passphrase when nothing is saved and refusing — rather than hanging — when there is no terminal to ask on.

  The file uses a key-encryption-key / data-encryption-key split: one Argon2id derivation from the passphrase unwraps a random data key, and that key seals each entry with AES-256-GCM. Deriving per entry instead — which is what the machine-bound backend does, where the key input is a high-entropy machine key and lighter parameters are therefore enough — would make a single `retrieve` cost a full KDF pass and a fifty-entry migration cost fifty of them. The split also lets a wrong passphrase be detected by unwrapping the key rather than by guessing which entry to trial-decrypt, and it is what makes a future passphrase change rewrap 32 bytes rather than re-encrypt every credential.

  Because the file arrives from a shared folder, its header is treated as untrusted input rather than as something this program wrote: an unknown format version or key-derivation function is refused, and the Argon2 cost parameters are capped, so a file claiming 4 GiB of memory cost is rejected before anything is allocated. Entry blobs and the wrapped key are bound to their roles by AAD, so relocating one into the other's slot in the JSON fails authentication instead of quietly decrypting. Writes go to a temp file, are `fsync`ed, then renamed, with the directory synced too — this file is frequently the only copy of the user's passwords and frequently lives on a network or FUSE mount, where an unflushed rename can leave nothing behind.

  What it does **not** do is merge concurrent edits: each save rewrites the whole file, so two machines saving while offline resolve as last-writer-wins. The session key cache is fingerprinted on the file's salt *and* its wrapped key, so a file replaced by the sync client is noticed and re-derived rather than decrypted with a stale key — but the ceiling is real and documented in `docs/USER_GUIDE.md` next to the other thing that cannot be undone: there is no passphrase recovery, which is why setup asks for it twice.

### Fixed

- **macOS: the Dock showed a generic terminal tile instead of RustConn's icon** — macOS does not read the Dock icon from the running program; LaunchServices reads it from the launched bundle's `Info.plist`. A process with no bundle behind it therefore gets the generic Unix-executable tile, which is what a launch from a shell looks like. The window icon set with `set_icon_name` cannot help: it is an X11/Wayland concept and a no-op on the GDK macOS backend, and GDK exposes no Dock API. The tile image is now set at runtime through `-[NSApplication setApplicationIconImage:]` when RustConn is *not* running inside a bundle, using the 256×256 icon embedded in the binary — that is the size the Dock draws at its 128pt maximum on a Retina display, so nothing is upscaled.

  Inside a properly built `.app` the call is skipped on purpose. The bundle's `.icns` carries every representation from 16px to 1024px and preserves a custom icon pasted onto the app in Finder, both of which a single embedded bitmap would replace. Only the tile is settable this way in any case — the name under it, the Cmd-Tab entry and the application menu still come from the bundle, so `scripts/macos-build.sh` remains the way to get all of them.

  The AppKit call lives in a new `rustconn-dock-sys` crate, the fourth sanctioned location for `unsafe` in the workspace, alongside the PTY, `setlocale` and `setenv` helpers. Its precondition guard and outcome reporting are platform-independent and covered by nine tests that run on Linux CI; only the `setApplicationIconImage:` call itself is macOS-gated, since `objc2-app-kit` builds nowhere else.

- **macOS: the Homebrew formula's `.app` had no bundle identity of its own** — `packaging/macos/rustconn.rb` pointed `CFBundleExecutable` at a wrapper script that `exec`ed the keg's `bin/rustconn`, a binary outside the `.app`. LaunchServices started the script, the script replaced the process image, and what ended up owning the window was a process with no enclosing bundle: generic Dock tile, wrong name under it, and no LaunchServices scene for `NSStatusItem`, so the tray icon could not appear. This is the same root cause fixed for the canonical producer in 0.19.x; the fix had never been carried across to the formula, which is the file the release workflow copies into the Homebrew tap. The bundle now holds a real `Contents/MacOS/rustconn` that `CFBundleExecutable` names directly, its translations are installed bundle-relative under `Contents/Resources/locale` (previously supplied by the wrapper's `LOCALEDIR`), and the environment wrapper survives only as an optional manual launcher at `Contents/Resources/bin/rustconn-wrapper`, matching the canonical layout. The binary is a second copy rather than a link: a symlink would make bundle detection depend on whether macOS resolves it, and a hard link would be silently undone by any Homebrew relocation pass that rewrites the file instead of editing it in place.

- **Terminal starts at correct size instead of 24×80 (issue [#294](https://github.com/totoshko88/RustConn/issues/294))** — a freshly spawned session read the VTE widget's grid dimensions before GTK had allocated pixel space to it, so the PTY always received the default 24 rows × 80 columns. Programs that check geometry only at startup (`mc`, `htop`) rendered in a small rectangle regardless of the actual window size. The real dimensions are now pushed to the child on the first main-loop idle after the session is spawned, which in practice is after layout and well before the existing 250 ms poll would have noticed the mismatch. It is best effort by construction: if the idle does run before allocation, VTE reports its own 24×80 defaults and the poll fixes it a quarter-second later, so the cost of being early is a slower correction rather than a wrong size.

- **Keyboard shortcuts help dialog now reflects user overrides (issue [#295](https://github.com/totoshko88/RustConn/issues/295))** — the Menu → Keyboard Shortcuts window was built from a compile-time constant array and always showed default bindings, even after the user recorded a new combination in Settings. It now reads the effective accelerators from the running application, which already incorporates overrides applied at startup and after each Settings save, so a remapped shortcut appears correctly in both the settings page and the help dialog.

  The keybinding registry only holds actions the user can *remap*, so building the dialog from it alone dropped eleven shortcuts the application implements through widget key controllers — sidebar edit, delete, rename and duplicate, connection copy/paste, Return to connect, the three terminal font-zoom bindings, and `F10` for the primary menu, which the project's own GNOME HIG notes list as mandatory. They are listed again, from a separate table of non-rebindable shortcuts, and two tests now assert in both directions that the table and the registry together cover every accelerator the application has. The previous drift guard asserted only that the registry was a subset of the dialog, which became trivially true the moment the dialog was generated from the registry, and it was deleted along with the array rather than inverted.

  Grouping changed as a side effect: shortcuts are now organised by the registry's own six categories, so fullscreen, sidebar, compact mode, passthrough and broadcast have moved from **Application** into a **View** section.

### Improved

- **`rustconn-cli --backend` can now name either file backend** — `encrypted-file` and `portable` (plus `file`, `portable-file`). The machine-bound encrypted file already had working `get`/`set`/`delete` handlers, but `parse_backend` had no spelling that reached them, so the only way to use it from the CLI was to make it the preferred backend in Settings first.

- **The portable passphrase was missing from three places that decide what happens to a secret** — `has_new_runtime_secret` did not list it, so pressing Save in Settings with only the passphrase filled in was classified as "nothing changed" and skipped; the runtime-secret preservation block in `AppState::update_settings` did not carry it over, so any Settings save after a mid-session unlock re-locked the store from the GUI's point of view and prompted again on the next connection (the issue #259 failure mode that block exists to prevent); and the property-test helpers that exist to catch exactly this omission had been extended for two of the new fields but not the rest.

- **A wrong portable passphrase read as "no stored password"** — `SecretManager::retrieve` skipped unavailable backends and discarded backend errors, so a mistyped passphrase in Settings, or a file the sync client replaced with one under a different passphrase, produced a password prompt rather than an explanation. `IncorrectPassphrase` and `PassphraseRequired` now surface instead of being walked past; every other backend error stays absorbed, so a KeePassXC hiccup or one corrupt entry still falls through to the next backend, which is what the chain is for. Settings also verifies the passphrase against the existing store on save — `verify_portable_passphrase` already existed and only the migration button was calling it — and the credential *save* path now reports a locked store in the user's language with a route to Settings, instead of surfacing the raw core error string.

- **Selecting the portable backend made the machine-bound store unreachable** — the fallback registration excluded both file backends from the chain. For `EncryptedFile` that correctly avoids a duplicate; for the portable store it removed the only fallback, so every credential in `credentials.enc` became unreadable the moment the user switched — before they had a chance to run "Copy Credentials", and permanently if they never did. The user guide and the migration wizard both promised the originals stay readable as this machine's fallback; now they do.

- **A half-finished credential copy reported only a count, in a toast that then went away** — "Copied 12 credentials, 3 could not be read" told the user something needed attention and took away the only record of *which* three. The failing entries are named in a dialog now (capped at a dozen, with the full list in the log), together with what to do about them, per the project's rule that a user-triggered failure risking a half-finished action belongs in a dialog rather than a toast.

- **"Save passphrase" could silently not save it** — when no machine key could be derived, the local encryption failed, a warning went to the log, and Settings said nothing; the next start then asked for a passphrase the user had told it to remember. `apply_storage_persistence` now reports which backends it could not honour, the placeholder blob is cleared rather than left to be mistaken for real ciphertext, and the Settings save path shows the same blocking dialog it already shows for a failed keyring write — the failure has the same shape, a secret that exists in memory only.

- **An unrecognised modifier in the shortcuts list was rendered as "Ctrl"** — the fallback in `accel_to_human_readable` printed any token it did not know as `Ctrl`, so an unfamiliar binding was displayed as a different, plausible-looking shortcut with no way for the user to tell it apart from a correct line. Unknown tokens are now shown as written. Letter keys are also shown uppercase again (`Ctrl+Shift+C`, not `Ctrl+Shift+c`), which the hand-written table had done and the generated list had lost.

### Security

- **The portable store's own size is treated as untrusted input, like its header already was** — the header's Argon2 costs were capped, but the file was read into memory and parsed into an unbounded map *before* any of that could run, so the one allocation that happens ahead of all validation was the one that was not bounded. The file size is now checked by `metadata` before the read (8 MiB) and the entry count is capped (10 000). The Argon2 ceilings came down from 1 GiB / 16 / 16 to 256 MiB / 12 / 16 — still four times the defaults, but low enough that the worst case is survivable, which matters because there is no honest way to cancel a derivation once `hash_password_into` is running.

- **The temp file used for the atomic write is created `0600`, not chmod'ed afterwards** — the mode was set after the write and `fsync`, leaving a window in which the file was world-readable, and what sat in that window was the KDF salt and the wrapped key: exactly the material an offline attack on the passphrase needs. The mode now goes into the `open` call. The temp name is also random rather than a fixed `.tmp` sibling, and opened with `create_new`, so a pre-planted symlink in a shared folder is an error instead of a redirected write and two RustConn processes cannot clobber each other mid-write. Both fixes apply to the machine-bound store as well, where the same ordering was inherited.

- **Concurrent writes to one portable file no longer lose an entry** — every `store` and `delete` is a read-modify-write of the whole JSON, and two of them interleaving meant the second write dropped the first entry. `store_bulk` and the migration wizard both issue writes back to back, so this was reachable in a single process, not only across machines. A mutex now spans the whole sequence. Writes from *different* processes or machines are still last-writer-wins, which is documented rather than fixed.

- **A store declaring format version 0 is refused** — only versions newer than 1 were rejected, so a file whose `format_version` had been lost to a bad sync merge was parsed as v1 and would have been rewritten in that guess.

- **Machine-key material is wiped on drop** — `get_machine_key` and `derive_settings_key` returned bare `Vec<u8>` and `[u8; 32]`, leaving each caller to remember to wrap them; two did not. Both now return `Zeroizing`, which is what the new passphrase derivation already did, so the two sit side by side in the same module saying the same thing.

- **The portable store can be locked again without restarting** — `clear_passphrase` had no caller outside tests, so a passphrase kept for "this session only" lived for the whole process. Locking now clears the settings copy, the backend's passphrase, its cached data key and the credential cache, the last of which would otherwise keep answering lookups after the lock and make it look as though nothing had happened.

- **The Settings passphrase check no longer runs on the GTK main thread** — "Copy Credentials" verified the passphrase inline, so a store with high KDF parameters froze the whole dialog. It runs on a blocking thread now, matching what the unlock dialog already did.

- **`rustconn-cli` asks twice before creating a new portable store** — there is nothing to verify a passphrase against when the file does not exist yet, so a typo silently became the passphrase of a file that the intended one would never open, with no recovery. The GUI already guarded this with a confirmation entry; the CLI now does too.

### Localisation

- **Thirty-two new strings translated in all 16 locales** — and the reason to mention it: `msgmerge` fuzzy-matched eighteen of them against unrelated existing entries, and its guesses were not near-misses. "Cannot open the file: {}" came back as the Ukrainian for "Host check failed", "Incorrect passphrase" as "Save passphrase", "Existing passwords" as "Enter password". `scripts/check-po-complete.sh` exists precisely because gettext renders a fuzzy entry as English while `msgfmt --statistics` still counts the catalogue as populated; it earned its place again here. The Ukrainian was reviewed against the project style guide, which replaced the literal "машинно-зв'язаний" for *machine-bound* with the wording already used in the dialog body, so the same idea reads the same way in both places.

- **Wrapped literals inside `i18n()` reach the catalogue with their source indentation** — `xgettext --language=C` does not honour Rust's `\<newline>` continuation, which collapses the newline *and* the leading whitespace. A wrapped translatable literal therefore produces a msgid full of runs of spaces that the runtime string never matches, so the translation can never apply. The three new strings that would have hit this are written on one line. Four pre-existing strings still have it (in `variables.rs` and one connection-dialog subtitle) and are silently untranslated in all 16 locales; fixing them changes their msgids and needs 64 catalogue entries revisited, so it is left as its own change rather than folded in here.

### Documentation

- **`docs/USER_GUIDE.md`** — new "Portable Encrypted File (cloud-syncable)" section: setting it up on the first and second machine, what each "Save passphrase" choice does, and the four things to know before relying on it (no recovery, one writer at a time, the file is guessable offline if obtained, keep the machine-bound copy).
- **`docs/ARCHITECTURE.md`** — the key hierarchy, why it is not per-entry, the untrusted-header handling, why `SecretManager` holds a typed handle to this one backend, and the cloud-sync ceiling with its upgrade path.
- **`docs/CLI_REFERENCE.md`** — the two new backend aliases and how the CLI obtains the passphrase.
