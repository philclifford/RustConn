...
- **A connection can take its jump host from its group or from the application (issue [#301](https://github.com/totoshko88/RustConn/issues/301))** — the centralised proxy management the request asks for, without repeating the bastion on every connection. Three tiers are consulted, nearest first: the connection's own Jump Host / ProxyJump, then the group chain, then a new **Settings → Connection → Network** group holding a Global Jump Host and a Global ProxyJump. The global tier exists because a group cannot stand in for "everything" — a top-level group is just a group with no parent and there can be many of them, and an ungrouped connection has no chain to walk, so nothing set on a group can ever reach it. A new **Network Mode** row in the connection editor chooses between *Inherit from group or global* and *Direct*; `Direct` refuses an inherited bastion but leaves one set on the connection itself in force. `rustconn-cli add`/`update` gained `--network-mode inherit|direct`. Both tiers offer a Jump Host picker as well as a text field, because a saved connection also carries its port, its identity file and its own bastion chain, none of which fit in `user@host`.

### Fixed

- **A group's Jump Host was never used (issue [#301](https://github.com/totoshko88/RustConn/issues/301))** — the picker in the group editor stored the choice, showed it again when reopened, and synced it, but nothing read it at connect time: `resolve_ssh_jump_host_id` had no callers at all. It is now resolved as the first hop of the chain. Two more defects in the same dialog: a group configured with *only* a Jump Host opened with the SSH switch off, and because the off branch clears all five SSH fields the next save silently threw the jump host away; and the Jump Host and ProxyJump rows were hidden unless an authentication method was chosen, though a bastion is how the group is reached and the credential its connections present is a separate question.

- **Whether a connection inherited its proxy depended on where it got its SSH key** — `resolve_ssh_proxy_jump` and `resolve_ssh_jump_host_id` walked the group chain only when the key source was *Inherit from group*. Nothing chose that; the flag was standing in for "the user configured SSH themselves", and it made a group bastion and a connection-level key file mutually exclusive. It was also applied inconsistently: for RDP, VNC and SPICE the SSH-config lookup returns nothing, so those protocols already inherited unconditionally, with no way to refuse. Inheritance now keys off the new Network Mode, which says what it means and reads the same for every protocol. Not one test asserted the old behaviour, which is how it survived; eleven new cases now cover inheritance with a connection-level key file, `Direct` at each tier, the global tier reaching an ungrouped connection, and a non-SSH protocol honouring the mode.

- **`rustconn-cli group set --ssh-proxy-jump ""` produced `ssh -J ""`** — a blank value was stored verbatim and survived all the way to the command line, breaking it rather than disabling the proxy, which is the obvious reading of passing an empty string. An empty value now clears the setting, for `--ssh-agent-socket` too, and a blank inherited from a group falls through to the next tier instead of being taken as a value.

- **A pinned connection's real row never updated (adjacent to issue [#302](https://github.com/totoshko88/RustConn/issues/302))** — a pinned connection is in the sidebar twice, once under the virtual "Favorites" group and once in its real place, as two separate row objects that share an id. Each row listens to its own object, and the four update walkers stopped at the first match — always the Favorites copy, since that group is first in the tree. The row in the group or at the root therefore had no status icon, no recording dot, no external-viewer emblem and no split marker. It went past the icons: the context menu reads the connected state from the row that was clicked, so right-clicking a pinned connection at its real place offered *Connect* for a session that was already open and hid *Stop Recording* while it was recording.

- **The recording dot, the external-viewer emblem and the split marker vanished on any sidebar reload** — for every connection, not only pinned ones. The connected/failed status was read back from the sidebar's own map when the tree was rebuilt; those three had nowhere to be read back from, so a rename, a duplicate, a pin toggle, a re-sort or a drag-drop cleared them until the next session event. They now persist across a rebuild like the status does.

- **The Simplified Chinese translation has never loaded, in any package** — since it was contributed in PR [#94](https://github.com/totoshko88/RustConn/pull/94). Every install path derives the locale directory from the catalogue's filename, so `zh-cn.po` was installed to `share/locale/zh-cn/`, while gettext looks up `zh_CN` and never the hyphenated form — ten paths affected, from the OBS spec and the Debian rules to the Flatpak manifests, the Nix package, the Homebrew formula and `install-desktop.sh`. The application had the right locale all along, which is exactly why the mismatch was invisible from the code. The catalogue is now `po/zh_CN.po`; a stored `zh-cn` language choice keeps working.

- **`Settings` populated its connection dropdowns before it was given the connection list** — `set_connections` ran after `set_settings`, which is what fills them, so the startup-action picker had nothing to choose from.

### Changed

- **The sidebar's "Open new session" is offered whenever there is a session to duplicate (issue [#302](https://github.com/totoshko88/RustConn/issues/302))** — the action already force-launched a second, independent session for a connection, bypassing the double-click that focuses the existing tab. It was only in the menu for external-viewer sessions, so for an embedded session it was unreachable and the only alternative was the global "Open a new session on every double-click" preference, which changes what *every* double-click does. Now shown whenever the connection has a session, embedded or external.

- **A server that runs RDS licensing is named as such instead of "incompatible"** — a Windows host with Remote Desktop licensing fails the embedded client's licensing exchange and the session is handed to the external FreeRDP client, which connects. That behaviour is unchanged and correct; what changed is that the banner now says the server requires RDS licensing rather than that it is incompatible, and the log names the phase. The cause is upstream and open ([IronRDP #1629](https://github.com/Devolutions/IronRDP/issues/1629)): the host sends an auto-detect probe during the licensing exchange and the connector feeds it to the licensing decoder without checking the channel. Not the same as [#1457](https://github.com/Devolutions/IronRDP/issues/1457), which relaxed a different header check and is in any case unpublished — `ironrdp-pdu` on crates.io is still 0.9.0.

- **An OpenH264 library that is installed but refused now says why, and can be overridden** — `Invalid hash: 55c8052d…` was the whole explanation. The loader compares the library's SHA-256 against a list of the binaries Cisco itself publishes and refuses anything else, deliberately: Cisco pays the H.264 patent royalties for what it distributes, which is why the upstream crate documents downloading the library from Cisco rather than packaging it. No distribution build can be on that list — `libopenh264-8`, Fedora's `libopenh264` and a local build from the Cisco *source* tarball are all refused, and the unchecked loader is not exposed. The message now says this, and `RUSTCONN_OPENH264` names a library to try before anything else, so a user who fetches the Cisco binary can enable H.264 without root. The 0.20.6 entry claiming "Flatpak was never affected: it builds its own copy" had it backwards — building its own copy is exactly why Flatpak fails the same check. Documented in `docs/INSTALL.md`.

### Improved

- **Every Secret Service call has a deadline, not just the GUI's whole credential resolution** — reading a secret from a locked keyring makes the Secret Service raise its own unlock prompt and answer only once it is dismissed, with no deadline of its own. The GUI bounded the entire resolution at 30 s, one budget shared by five lookup keys and every round-trip they make, and could not say which step stalled; `rustconn-cli` and any other consumer of `rustconn-core` had no bound at all and could wait forever. Each call now gets the project's standard 10 s vault budget and names itself in the log and in the error. This covers the four keyring helpers whose Linux branch had no budget while the macOS branch did — the ones that carry the Bitwarden master password, the 1Password token, the Passbolt passphrase, the KDBX password and the portable-store passphrase.
...
- **Two new UI strings would have been translated and still rendered in English** — they were written as continued Rust literals. `xgettext` reads these files as C, where a backslash-newline splices the literal and keeps the indentation that follows, while rustc strips it, so the msgid carries whitespace the program never sends and the lookup misses. Both are now single lines. Thirteen strings elsewhere in the tree still have this defect and are recorded for a follow-up, since fixing them changes msgids and touches all 17 catalogues.

- **rpmlint errors on the OBS build cleared** — the RPM `Summary` was 104 characters against a limit of 79 and had drifted from the Debian description by gaining MOSH; the `.mo` files carried no `%lang()` marker, so `%find_lang` now generates the list and owns the directories, retiring the hand-maintained `%dir` entries; `Requires: libadwaita` and `libasound2` are gone, since rpm derives both from the linked binary and `libadwaita` is not even a package name on openSUSE; and a `%check` section runs the `rustconn-core` suites. `unstripped-binary-or-object` is left alone deliberately, with the reason recorded in `Cargo.toml`: silencing it needs `strip = "symbols"`, which would take the function names out of every panic backtrace, and this project is debugged from logs users attach to reports.

- **`Recommends: libopenh264` stays, and the comment now says which client it is for** — the external FreeRDP fallback uses it for `/gfx:AVC420` and accepts a distribution build; only the embedded client refuses one.
