...
- **New strings localised in all 16 languages** — the 13 user-facing strings the detach feature adds (menu items, per-monitor labels, the detached window title, the failure toasts, the shortcut description) are translated in be, cs, da, de, es, fr, it, kk, nl, pl, pt, sk, sv, uk, uz and zh-cn, so the feature ships localised instead of falling back to English.
