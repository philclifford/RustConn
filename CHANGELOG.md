...
- **A `wc -l` measurement made the cache-hit check silently measure nothing on macOS** — BSD `wc` pads its output, and BSD `tail` then rejects `-n +      18` as an illegal offset, so the check that looks for `Checking`/`Compiling` lines in clippy's output scanned an empty stream and concluded clippy had compiled nothing. Harmless while that was a warning, a false failure the moment it became one.
