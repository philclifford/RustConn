...
- **Terminal history now survives a reconnect (issue [#253](https://github.com/totoshko88/RustConn/issues/253))** — a session dropped by a server idle timeout came back to an empty terminal, so everything the user wanted to consult was gone at the moment they asked for it back. Reconnect already reused the same VTE widget and the same tab; it simply called `reset(true, true)`, whose second argument tells VTE to empty the scrollback — while `mark_tab_disconnected`, fifty lines below, deliberately used `reset(true, false)` to keep that scrollback readable under the disconnect banner. An in-place reconnect (SSH, Telnet, Serial, Kubernetes, Mosh and custom-command sessions) now keeps it too, with a dim `── Reconnected at … ──` rule opening a fresh line so the preserved output and the new session stay apart. Two details had to come with it. `reset` switches back to the normal screen only in its `clear_history` branch, so a session that died inside a full-screen app (vim, htop, less) would have kept showing that app's frozen screen and hidden the very history just preserved — `DECRST 1049` is now fed after every history-preserving reset, the one at disconnect included, which also makes the scrollback readable while the tab sits disconnected. And the viewport returns to the bottom, because the user may well have scrolled up to read the dead session. "Keep on reconnect" in Settings → Terminal → Scrolling turns the whole thing off; it is on by default.

- **Automatic login for Telnet and serial sessions, with configurable expected prompt text (issue [#254](https://github.com/totoshko88/RustConn/issues/254))** — a Telnet connection resolved its password from the vault like any other connection and then never used it: `start_telnet_connection_internal` did not read the credential cache at all, and the only auto-fill in the program (`looks_like_password_prompt` plus a VTE watcher) was wired into the two SSH paths exclusively. Neither Telnet nor Serial ever typed an account name for any protocol. Both now log in the way those protocols actually authenticate — by typing at the device's own prompts — with the account name taken from Username and the password from the configured Password Source; a connection with neither set is untouched. Because vendors word those prompts inconsistently (`>>User name:` on a Huawei OLT MA5800, `Username:` on an S6700, `login:` on a Datacom), the Automation tab gained an **Automatic Login** group with **Username Prompt** and **Password Prompt** fields, matched as a case-insensitive substring of the line under the cursor rather than as a regex — regex already has a home in Expect Rules. All three examples above, plus `login as:` and `user:`, are recognized with both fields left empty, so the fields are the exception rather than the setup step. The same two fields exist on a group (Edit Group → **Automation**) and are inherited field by field down the group chain, which is what a folder of identical devices from one vendor needs; on the CLI they are `rustconn-cli group edit --username-prompt/--password-prompt`. Each step of the sequence fires exactly once: a device that rejects the credentials and re-prompts is handed back to the user, because automatic retries are how an account gets locked out. `Last login:` in an MOTD is explicitly not a username prompt.
...
- **`${password}` and `${username}` in an expect-rule response are documented as global-variable-only** — the Automation tab and the group editor both promised that those two placeholders "resolve at connection time", and the user guide went further and claimed `${password}` is "automatically filled from the connection's configured secret backend". Neither was true: `prepare_rules_from_config` substitutes against `VariableScope::Global`, seeded exclusively from `settings.global_variables`, so an undefined `${password}` was replaced with an empty string and the built-in "Login Prompt" template silently sent nothing but a newline. Routing a real password through that path is also a dead end rather than a missing feature — `substitute_for_command` rejects values containing shell metacharacters, so any password with a `$`, `;` or `!` would fail substitution and leave the literal `${password}` to be typed into the session. The documentation now says what the code does and points at Automatic Login, which reads the password from the secret backend directly and never puts it in a config file.
...
- **The file-manager SFTP path now says when it cannot work** — the GVFS sftp backend spawns its own `ssh` with a hardcoded argument list (`-oControlMaster`, `-oControlPath`, `-oForwardX11 no`, `-oNoHostAuthenticationForLocalhost`) and accepts no `-F`, and it is D-Bus-activated so its `PATH` is not ours to set; a jump host cannot be applied there at all. Choosing an external file manager for a connection that has one now raises a warning pointing at the "SFTP via mc" setting instead of failing silently. The same warning is logged by `rustconn-cli sftp`.
