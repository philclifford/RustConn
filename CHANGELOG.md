...
- **"Enable fallback" is now "Also read from the encrypted file", and governs reads only** — the description said "Use libsecret if the primary backend is unavailable", naming a store that stopped being the fallback when `build_from_settings` moved to `EncryptedFileBackend` (#201) and was never updated. It now names the encrypted file, is no longer platform-conditional, and says what it does: look there as well when resolving a password, so entries saved before a backend change keep working. Where a *write* goes is no longer part of this setting — see **Fixed**.
...
- **`SecretManager::retrieve_reported` reports which backend answered a read** — a `RetrieveOutcome`, the read-side twin of `StoreOutcome`, so a caller can tell a password from the selected backend from one out of the encrypted-file fallback. `retrieve` delegates and discards the outcome, so its contract is unchanged, and a fall-through is logged with both backend ids. Not yet surfaced in the interface: the remaining chain reads go through `CredentialResolver`, which would have to carry the outcome too.
