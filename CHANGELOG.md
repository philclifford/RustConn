...
- **Detachable session windows (issue #236)** — a session can be moved out of its tab into its own top-level window and back again, which is what a multi-monitor desktop actually needs. Supported for every protocol RustConn renders in-process: VTE sessions (SSH, local shell, Telnet, Serial, Kubernetes, Mosh, ZeroTrust) plus embedded RDP, embedded VNC and the embedded Web viewer. Reachable from the tab context menu ("Move to New Window"), from `Ctrl+Shift+M` — which toggles in both directions depending on which window has focus — and, when more than one monitor is connected, from a "Move to New Window on…" submenu that opens the window fullscreen on the chosen monitor. The detached window carries a header-bar button that moves the session back into a tab. The move never reconnects the session: the same live widget is reparented, so scrollback, monitoring, recording, and tunnels survive it. Closing a detached window ends its session exactly as closing its tab would, and closing or quitting the main window takes detached windows with it — the close/quit confirmation now counts detached sessions, so a window whose only remaining session lives on another monitor still warns before it goes down. Re-activating RustConn (second instance, D-Bus or tray activation) presents the main window instead of whichever window happens to have focus. `Ctrl+Shift+M` is listed in the keyboard-shortcuts window and, being a normal registry entry (`win.toggle-detach`), can be remapped like any other binding. Deliberately not offered for external-viewer sessions (SPICE, RDP/VNC in external mode), whose display already runs in its own operating-system window, nor for split tabs, which must be unsplit first.

### Fixed

- **Split view could steal a session out of its detached window (issue #236)** — the "Select Tab" picker of a split pane listed sessions by protocol eligibility only, so a session running in its own window was offered there; picking it moved the live widget into the split panel and left an empty registered window behind, whose close handler then ended a session that was by then visibly running in a tab. Detached sessions are now filtered out of both split pickers, and the "Select Tab" handler asks the shared placement rule before it touches the widget, so any other route into it is refused (with a warning in the log) rather than corrupting the placement first and reporting it afterwards.
- **Detaching the only tab left the main window empty (issue #236)** — moving the last session to its own window removed its tab without bringing back the Welcome tab, so the main window showed a blank content area until a new session was opened. Parking the last tab now recreates the Welcome tab exactly as closing the last tab does, honouring the "Show Welcome tab" preference.
- **A detached session was cut off from disconnect handling and auto-reconnect (issue #236)** — the reconnect banner, its retry countdown, and the reconnect button were placed by way of the tab, so a dropped connection in a detached window showed a dead terminal with no explanation and no way back, while background auto-reconnect skipped the session entirely as "tab closed". Session feedback now follows the session into whichever window holds it, auto-reconnect treats a detached session as alive, and an in-place reconnect brings the session back up inside its own window instead of closing it and opening a new tab.
- **Detached sessions disappeared from the session manager and from saved workspaces (issue #236)** — both lists were built from the tab map, so a session in its own window had no row, was missing from the "UI session(s)" count, and was silently dropped when a workspace was saved. Detached sessions are now listed and counted like tabbed ones (their row says the session is in a separate window, translated in all 16 languages) and are saved as ordinary workspace entries, restored as tabs, in a stable order instead of one that changed on every run.
- **A session moved back into a tab lost its tooltip and group label (issue #236)** — the recreated tab set only the title and the protocol icon, so the hover tooltip and the group name it carries were gone until the tab was regrouped by hand. The tooltip, its host line, the group suffix, and the group title prefix now come back with the tab — the host travels with the session, so an embedded VNC tab keeps the same two-line tooltip it was created with — and a session whose metadata has vanished keeps its current placement instead of ending up with neither a tab nor a window.
- **A dropped connection in a detached embedded RDP or VNC window looked like a working one (issue #236)** — those protocols cannot reconnect in place, so they get no reconnect banner, and a detached session has no tab to carry the offline indicator either: the window simply kept showing the last frame. Such a window now raises a "Session disconnected" banner of its own, and clears it when the session comes back.
- **A detached session that reconnected came back as a tab (issue #236)** — for protocols that reconnect by restarting the session (embedded RDP, VNC, Web) the reconnect closes the session and creates a new one, which took the detached window down with it. The placement is now remembered across that step and the reconnected session is moved back into its own window.
- **Renaming a connection left open sessions showing the old name (issue #236)** — the rename reached the sidebar only: open tabs kept the previous title and tooltip, and a detached window kept it in its own title bar. A rename now updates every open session of that connection, in tabs and in detached windows.
- **A failed attach could leave one session in two places (issue #236)** — if the content could not be rebuilt while moving a session back into a tab, the session ended up addressable both as an empty tab and as its own window. That path now rolls all the way back to "detached", drops the empty tab, and resumes monitoring, so a failed move changes nothing.
- **Activity and silence toasts for a detached session appeared on the main window (issue #236)** — a notification about a session is now shown on the window that actually holds it, instead of being delivered somewhere the user is not looking.
- **Quit confirmation appeared on the wrong window (issue #236)** — `Ctrl+Q` pressed in a detached window opened the "close open sessions?" dialog on the main window, which may be behind other windows or hidden in the tray. The dialog is now parented to the focused visible session window, with the main window as the fallback, so it can no longer land on a hidden window or on another dialog.
...
- **Dead `external_window` module (`ExternalWindowManager`)** — a 288-line abstraction for hosting terminals in extra windows that `MainWindow` constructed and then never used: no menu item, no action and no keybinding ever reached it, and the field only compiled because of an `#[expect(dead_code)]` on the whole struct. The detachable session windows above are a fresh implementation (`detached_window.rs`), so the stub and its `SharedExternalWindowManager` alias are gone, and dropping that blanket expectation means every remaining `MainWindow` field is now genuinely in use. Nothing user-visible disappeared — there was nothing to reach.
...
- **One code path builds a session's tab content** — `reparent_terminal_to_tab` both assembled a tab's widget tree and moved the live session widget into it, which the detach/attach path would have had to duplicate. It is now split into `build_session_content` (terminal or embedded viewer, monitoring bar, toolbars) and `park_tab_page` (removes the page and marks the session as living elsewhere), so a session moved back from its own window is rebuilt exactly like a session restored from a background tab. No behavior change for tabs; the monitoring bar is suspended and resumed around a move the same way the split path already did it.
- **Menu wording and detached-window title follow the HIG (issue #236)** — the multi-monitor entry is now "Move to New Window on" without an ellipsis, since it opens a submenu rather than a dialog, and its neighbour "Set Group…" uses the proper single-character ellipsis it always should have. A detached window opened for a connection with no name is titled by its protocol instead of starting with a stray dash.
- **Placement tests state the contract instead of restating the code (issue #236)** — the detachability property tests compared the predicate against a second copy of the same if/else chain, so a precedence change mirrored in both would have passed unnoticed. They now assert properties that stand on their own — a session is detachable exactly when nothing blocks it, and a rejection always names a blocking condition the session actually has — while the precedence order itself is pinned by name in the unit tests. The question "may this session be placed in a split?" moved into `rustconn-core` next to the detach predicate, so the two split pickers, the handler that commits their choice, and the placement model all answer it the same way, with a test for the transition they block. The display-dependent notebook checks are written into the quality gate, one process per test — they initialise GTK and cannot share one — so the split-guest regression is actually run.
- **New strings localised in all 16 languages** — every user-facing string the detach feature adds (menu items, per-monitor labels, the detached window title, the failure toasts, the shortcut description, and the session-manager row label "{} — in a separate window") is present in `po/rustconn.pot` and translated in be, cs, da, de, es, fr, it, kk, nl, pl, pt, sk, sv, uk, uz and zh-cn, so the feature ships localised instead of falling back to English.
