...
- **Session logging significantly improved (issue [#247](https://github.com/totoshko88/RustConn/issues/247))** — the output capture now uses a line-by-line diff against the previous VTE buffer snapshot with a 1-second sampling interval (down from 5 seconds), ensuring command output (`ls`, `df -h`, `cat`) appears in the log within 1 second. ANSI escape sequences are stripped from log files so they are human-readable in any text editor. Trailing blank lines from VTE viewport padding are trimmed. A unified PTY spawn path (`openpty` + `set_controlling_terminal`) replaces both VTE's built-in `spawn_async` (Linux) and the macOS-specific native PTY, resolving cross-platform inconsistencies. The initial connection burst (SSH negotiation, MOTD, login banner) is still captured within 500 ms via the one-shot timer from 0.19.13.
