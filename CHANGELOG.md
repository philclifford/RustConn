...
- **CI builds and tests macOS, for the first time** — a `macos-sys` job that clippies and tests the four `rustconn-*-sys` crates on a macOS runner. Every other job in the matrix is Linux, and until now nothing outside the maintainer's own machine had ever compiled the macOS side. That had already cost something: the `rustconn-pty-sys` contract test proving the `pre_exec` hook runs in the forked child — the thing that makes SSH password prompts work ([#175](https://github.com/totoshko88/RustConn/issues/175)) — had *never* passed on macOS, because it accepted `ENOTTY` and `EPERM` while macOS answers `ENODEV`, and it took until 0.20.11 for anyone to notice. A test guarding `unsafe` that no job executes is a test whose state nobody knows.

  Scoped to the four helper crates deliberately. They hold every `unsafe` block in the repository, they are where the platform differences live, and between them they need exactly one Homebrew package, because their dependencies are `libc`, `nix`, `gettext-rs` and `objc2`. Building the GUI there would mean gtk4, libadwaita, vte3 and WebKitGTK from Homebrew for a twenty-minute job — and `web-embedded`, a *default* feature, cannot build on macOS at all, which is a separate thing worth fixing before the scope grows. `rustconn-dock-sys` is the crate that gains most: off macOS its AppKit call is `cfg`-ed out and it contains no `unsafe`, so this is the only place its `unsafe` block, its main-thread guard and that guard's new contract test are compiled and run.

  Nine places in the code and docs asserted "no CI job builds macOS" and were used to justify design decisions. All nine now say what is actually true, which is narrower: one job covers the helpers, the rest of the matrix is Linux, and that is still the reason not to add a macOS-gated crate.

### Fixed

- **A Jump Host set on a group or globally was still ignored at connect time, and 0.20.9 said it had fixed that (issue [#301](https://github.com/totoshko88/RustConn/issues/301))** — the three-tier resolver shipped in 0.20.9 and is correct: the connection's own value, then the group chain, then Preferences → Network, with `Network Mode: Direct` refusing the inherited tiers. It has seven tests and they all passed. What was never wired is the *launcher*. The free-text ProxyJump field was routed through the resolver everywhere; the **picker** was read straight off the connection's protocol config at every launch path. So a bastion chosen from a dropdown above connection level was stored, shown in the editor as inherited, synced between machines, and then dropped at the moment it was needed. The 0.20.9 notes claimed inheritance "reads the same for every protocol"; it was true of neither form for RDP, VNC and SPICE, which read only the connection's own field, nor of the picker for SSH.

  Seven call sites, not the four the launch gates account for. Besides the SSH terminal and the RDP, VNC and SPICE tunnel gates, two `has_jump_host` guards and `bastion_may_prompt_for_password` inherited the text field while reading the picker raw — so a connection reached through an inherited bastion reported having none. That drives the status and monitoring paths, and the auto-fill suppression that stops the target's password being fed to the bastion prompt ([#191](https://github.com/totoshko88/RustConn/issues/191)). Fixing only the gates would have left the application dialling a bastion it believed was not there.

  All seven now go through one function, `window::protocols::resolve_first_hop_id`, which adds the two pieces of application state the core resolver needs and nothing else. Only the *first* hop resolves this way; hops further out keep reading their own field, which is what `resolve_jump_chain` already documents and is deliberate — a bastion's bastion is a property of that bastion, not something the target's group should redirect.

  **What this does not fix**: a *text* ProxyJump at group or global level still does not reach RDP, VNC or SPICE. Those protocols reach a bastion by opening an SSH tunnel, and a tunnel needs a saved connection with its own port, identity file and credentials — a `user@host:port` string has none of that. For them the bastion must be a picked connection, which now inherits. Said plainly here because "#301 is fixed" without that sentence would be the same kind of claim that kept the issue open.

  The precedence logic was tested and the wiring was not, which is why a correct resolver and a false changelog entry could coexist for two releases. `scripts/check-jump-host-wiring.sh` now fails if any file under `rustconn/src/window/` reads `jump_host_id` off a protocol config, in CI and in `release.sh`. It is a grep rather than a test on purpose: the thing that rots is a connection between a resolver and its callers, and covering it properly would need a GTK window, an `AppState` and a real bastion.

- **Both Flatpak `cargo-sources.json` manifests were three lock-file changes behind, which would have made the Flatpak build vendor crates it then could not find** — `Cargo.lock` moved three times this release: the tray feature change took 39 crates out, then quick-xml and argon2 were bumped, for a net 733 → 695. Neither `packaging/flatpak/cargo-sources.json` nor `packaging/flathub/cargo-sources.json` was regenerated at the time, because `flatpak-cargo-generator` needs both the tool and network access and the machine the audit was done on had neither. Both still listed 1455 entries, 15 of them GTK3 crates that had just been removed from the graph. Regenerated to 1377 entries, byte-identical to each other as the gate requires. Worth noting which gate caught it and how close it came to not being caught: `release.sh` checks exactly this, and it was the *single* gate that stood down for a missing tool during preparation — the same skip-accounting block added earlier in this release is what made a silently-skipped gate legible instead of one `[warn]` line among several hundred.
...
- **Four comments described behaviour the code does not have** — found by reading the code beside the claims rather than by a failure, which is how this class survives.

  `terminal::recording::flush_active_recordings` said it sends `exit` to each recording session. It sends EOF, `\x04`, through `detach_recording` — and the difference is the reason EOF was chosen, since it leaves no visible echo in the user's scrollback and is harmless if the sub-shell has already gone.

  `embedded_rdp`'s `terminate_external_process` documented itself as terminating the process "gracefully, waiting for it to exit", above a `child.kill()`, which on Unix is `SIGKILL` and cannot be caught. There is no grace period and nothing to wait out; the reap that follows only prevents a zombie. `external_session::disconnect` had the mirror-image problem: a `// ponytail:` note offering to "upgrade to `nix::sys::signal` if a `SIGTERM`→`SIGKILL` escalation is ever needed", which read as an unexplored option long after `terminal::child_teardown` had implemented exactly that escalation — process group, PID-reuse guard and all — one module away, using a `nix` that is already a dependency. Both now state the decision instead of implying an omission: a session child gets the grace period because it may be a shell holding an editor with unsaved work, while an external viewer is a window onto a desktop that lives on the remote host and has no local state a clean exit would flush.

  `rustconn-dock-sys` said `unsafe_code` is "forbidden" in `rustconn`. The workspace moved to `deny` in 0.20.0 precisely so the FFI crates could re-open it for themselves; `rustconn` keeps a crate-local `forbid`.

- **`rustconn-dock-sys`'s main-thread guard had no test, and it is the precondition that makes the crate's one `unsafe` call sound** — the suite covered the PNG signature check and the shape of every outcome, and never the guard. `refuses_a_call_from_another_thread` now asks it from a spawned thread and asserts `OffMainThread`. Spawned deliberately: libtest under `--test-threads=1` and nextest with its process per test both run a test body *on* the main thread, where the guard would pass and running the suite would replace the developer's Dock tile as a side effect. This is the gap that let a `rustconn-pty-sys` contract test sit failing on macOS until 0.20.11 — no CI job builds macOS, so an unexercised guard there is a guard nobody knows still works.

- **The macOS tray dragged the whole GTK3 binding stack into `Cargo.lock`, and with it nine of the ten advisory warnings** — `tray-icon` and `muda` were taken with their default features, which are `["libxdo", "gtk"]`, and `tray-icon`'s `gtk` feature is `["muda/gtk", "dep:libappindicator"]`. That put `libappindicator`, `gtk` 0.18 and its `atk`, `gdk`, `gtk-sys`, `gtk3-macros` and `glib` 0.18 into the lock file, along with `libxdo`, `x11`, `system-deps` and the rest of their build machinery: 39 crates, 733 down to 694. Both are now taken with `default-features = false`.

  None of it was ever *compiled*: those dependencies are already target-gated to Linux and the BSDs inside `tray-icon` and `muda`, and the two crates are only enabled at all by the `tray-macos` feature, which only the macOS bundle build passes — the Linux tray is `ksni` and pure D-Bus. But `cargo audit` reads `Cargo.lock`, which is target-agnostic, so the advisories were reported on every platform regardless. That is why target-gating the two declarations here would have fixed nothing and turning the feature off fixes it: the feature is what pulled the optional dependency into the graph.

  Measured before and after on the same tree: `cargo audit` went from ten warnings to one. The nine that went are RUSTSEC-2024-0412, -0413, -0415, -0416, -0418, -0419 and -0420 (the GTK3 bindings, unmaintained), RUSTSEC-2024-0429 (`glib` unsoundness) and RUSTSEC-2024-0370 (`proc-macro-error`, reached through `system-deps`). The one that remains is RUSTSEC-2023-0089, `atomic-polyfill`, which arrives by another route. Worth stating plainly because it was assumed otherwise while planning this release: **none of the nine was ever in an allow-list.** `.cargo/audit.toml` and `deny.toml` ignore exactly one advisory between them, RUSTSEC-2023-0071, and they still do — there was nothing to delete, and the warnings were being counted and tolerated rather than named and accepted.

- **The stale-version gate failed the release over a comment that has to name the old version** — it scans the version-only packaging files for the previous release number, which is a real defect it exists to catch: `debian.dsc` once carried `Version: 0.19.9-1` beside `Files: rustconn-0.19.8.tar.xz` for a whole release. But it read every line, and this release added a note beside the new Flatpak mirror list recording that `ftp.gnu.org` was unreachable during the *0.20.11* build — a sentence that stops being true if the number is bumped. Comment lines are skipped now, for the same reason changelogs are excluded from the scan altogether; commented-out YAML or TOML is inert, so nothing that matters can hide behind it. Verified in both directions: a planted `0.20.11` in live YAML is still caught.

- **`release.sh` ended a run that stood three gates down exactly the way it ended one that ran everything** — each skip was reported, as an individual `[warn]` line on stderr, interleaved with several hundred lines of other output, which is to say it was reported and not read. The run then printed the release plan and "validation passed". On macOS, the platform releases are actually prepared on, three gates silently stepped aside: the spell check, the cross-format release-date check and the `cargo-sources.json` check. The first of those is not hypothetical — v0.20.1 was tagged, pushed and published with a red CI over one Spanish word, and the fix at the time was to make `release.sh` run `typos`, which it does, right after checking whether `typos` exists and warning if it does not. The script now counts every gate it did not run and prints them as a block immediately before the plan, where the operator is looking, with a line saying that a gate which did not run has told them nothing. When nothing was skipped it says that too, so the absence of the block is not what has to be noticed.

- **The cross-format release-date check no longer skips anywhere** — it verifies that the date in `CHANGELOG.md` matches the Debian changelog trailer, the OBS `.changes` header and the spec `%changelog` header. Two formats have to be normalised, RFC 2822 and `Sat Aug 01 2026`; GNU `date -d` parses both and BSD `date` parses neither, so the check stood down on macOS. It now falls back to `python3`, which is present on both platforms and parses both formats out of its standard library. Reporting a skip is second best to not having one.
...
- **The check for whether clippy compiled anything was reading the wrong part of the log** — it recorded the shared log's line count before the run and then read from that offset afterwards. Wrong twice over: BSD `wc -l` pads its output, so `tail` rejected `-n +      18` as an illegal offset and the check scanned an empty stream; and with the padding stripped the offset still resolved to the end of the file, so it read a single line. Both times it announced that clippy had compiled nothing while the log beside it held several hundred `Checking` lines. Harmless while that was a warning, a false failure the moment it became one. Clippy's output now goes to a file of its own, holding exactly one run, and the check greps that — there is no offset left to get wrong. Verified in both directions: a cache hit is detected, and a run that recompiled one crate is not mistaken for one.

- **A translatable string could go missing from the template and no gate would notice — for the fourth release running** — every i18n check the project had reads the *committed* `po/rustconn.pot` and the catalogues beside it. None regenerated the template to compare against the sources, so a string that was never extracted was invisible to all of them: `check-potfiles.sh` confirmed the file was listed, `check-i18n-escapes.sh` confirmed the literal was extractable, and `check-po-complete.sh` reported every catalogue 100% complete — complete with respect to a template that was itself incomplete. The string rendered in English in every locale. 0.20.0 found thirteen strings like that and wrote in its own changelog that "nothing yet checks that the committed POT matches the sources; that gap is real and outlives this fix"; 0.20.1 found three more and repeated the sentence verbatim; 0.20.2 found two from [PR #286](https://github.com/totoshko88/RustConn/pull/286). `scripts/check-pot-current.sh` closes it, wired into the CI `i18n` job and into `release.sh`. It re-extracts by calling `po/update-pot.sh` rather than repeating the keyword list, because two copies of that list is precisely how a string ends up found by one extractor and missed by the other — the failure the gate exists to catch, reproduced in the gate. Source references are deliberately not compared: they move on every code edit and say nothing about translatability, and a gate that fails on unrelated work is a gate that gets switched off. Each direction is reported separately — a string in the sources but not the template, and a template entry no source contains any more.

  The gate also refuses to pass on an empty comparison, which is not hypothetical: the first draft used `msgcomm --unique`, which returns the symmetric difference rather than "in A and not in B" and so reported the same string as both missing and stale, and the second used a `msgcat --omit-header` option that does not exist — both sides came back empty, the sets matched, and a string genuinely absent from the template passed. Producing no msgids is now a failure rather than a match.

- **`po/update-pot.sh` aborted on macOS, after writing a template with a defect in it** — the last step strips a false `#, c-format` flag that `xgettext --language=C` puts on the command-palette string, reading `% tabs` as a format specifier. It did that with `sed -i` in the GNU spelling, where the backup suffix is glued to the flag. BSD `sed` takes the suffix as a separate argument, so it read the script as the suffix and the filename as the script, then waited on stdin; the multi-line `N` plus `\n` substitution was unusable there for a second reason. Under `set -e` the script exited non-zero *after* the template had been written, leaving a POT carrying the spurious flag — and `AGENTS.md` names this script as the thing to run after adding a translatable string, so the documented workflow was broken on the maintainer's own platform. Rewritten in `awk`, which needs no portable-`sed` incantation at all. Same class as the `check-i18n-escapes.sh` shebang fixed in 0.20.11.

- **The same sources produced two different templates depending on whose shell ran the extractor** — `xgettext` emits entries in the order it first meets them, so the file list decides the layout of the template, and that list was built with a bare `sort`. Under `en_US.UTF-8` punctuation is weighted down and `dialogs/connection_wizard/…` sorts ahead of `dialogs/connection/…`; under `C.UTF-8` the byte order puts `/` (0x2F) before `_` (0x5F) and the two swap. Identical msgids, several thousand lines of reordering. Invisible while the file was only ever regenerated by hand, and fatal to the new gate, which has to tell real drift from the machine it happens to run on. The sort is pinned to `LC_ALL=C`, and the template has been regenerated once so the committed file is the canonical form.

- **Every external process on a credential path could block for ever, and one of them held the whole Secrets page** — thirty-seven waits across five files had no deadline. The `keepassxc-cli` invocations were the worst of it: twelve call sites, every one an unbounded `wait_with_output`, in a project that bounds a credential resolution at 30 s overall and every Secret Service call at 10 s. Only the GUI's bulk-transfer path wrapped them, and that bounded the transfer rather than the child, so a database on an unresponsive network share, or one locked by another process, meant the calling thread never came back.

  The Secrets page was the visible symptom. Its probes run as scoped threads and the scope is not joined until the last of them returns, so one unresponsive CLI left the page empty rather than costing it a row — and the three most likely to stall are exactly the three that do I/O: `bw status` reaches the network once its session expires, `op whoami` can sit on a biometric prompt, `passbolt list user` talks to a server. All eight are now bounded at 5 s, where giving up reads as "not installed", which is already what a failed probe reads as.

  Reads and probes get 10 s; the invocations that *write* to the database — `add`, `mkdir`, `rm` — get 30 s, because the consequence of expiry differs rather than the expected duration. A killed read costs a lookup; a `SIGKILL` delivered to a write lands in the middle of rewriting the KDBX. The timeout message names key-derivation parameters as a possible cause alongside a locked file, since the KDF cost is paid on *every* invocation — `keepassxc-cli` reopens the database each run, so a hardened Argon2 configuration does not overrun once, it overruns at every call site, and a single save is four invocations.

  Also bounded: the three CUPS helpers on the RDP printer-redirection path (`lp`, `lpstat -e`, `lpstat -d`), which ran on the connection's own thread, at 2 s. One limitation there is now written down rather than implied — the document is written to `lp`'s stdin *before* the wait, and a PostScript page larger than the pipe buffer blocks in that write, which no deadline on the wait can cover.

- **Four copies of "wait for a child, kill it if it takes too long" became one** — `which`'s host probe, `connection::mdns`'s name resolution and `protocol::detection`'s version check had each grown their own fifteen-line poll loop, differing only in what they returned on expiry, and `secret::status` had none at all. The new `rustconn_core::proc::wait_bounded` reports `Exited(Output)` or `TimedOut` and lets the caller decide, which is the part that actually differed. It is tested for all four outcomes, including that a killed child is reaped rather than left as a zombie — the one thing none of the copies differed on and the thing a fifth copy would eventually have got wrong.

- **A timed-out detection probe left the process it gave up on still running** — `tokio::process::Child` does not kill on drop, so abandoning a probe at its deadline would have leaked the unresponsive process, once per refresh. The detection commands now set `kill_on_drop`.
...
- **argon2 0.5 → 0.6 — the key derivation behind the encrypted credential stores, and the question was whether existing files still open.** Deferred from 0.20.11 for that reason. They do, and it is now proved rather than assumed: the derivation is byte-identical, so no migration is needed and no stored credential is affected. It brings `password-hash` 0.5 → 0.6.1, `blake2` 0.10 → 0.11 and a new `phc` 0.6.1 with it, and drops `rand_core` 0.6.4; nothing in RustConn's own code had to change, since the algorithm, version and cost parameters were already passed explicitly.
  ...
  Proving it needed a fixture that did not exist. `derive_settings_key` has had a backward-compatibility fixture for a long time — a captured `RCSC` blob that fails to decrypt if the format moves — and `derive_passphrase_key`, which is what opens the *portable* store, had none. That gap mattered at exactly this moment: the round-trip tests create a store and open it with the same build, so they pass whether or not the derivation changed, while every store already on disk would have become unopenable with nothing going red. A fixed Argon2id vector is now pinned, captured under 0.5.3 before the bump and asserted to still hold after it, alongside a second test that pins the default cost parameters. Cheap parameters on purpose: Argon2id does not special-case cost, so a change in output shows up at 1 MiB as clearly as at 64 MiB, and the test costs milliseconds.
